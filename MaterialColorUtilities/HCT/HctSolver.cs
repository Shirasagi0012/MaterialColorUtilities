...
internal static class HctSolver
{
    ...
    private static double ChromaticAdaption(double component)
    {
        var af = Math.Pow(Math.Abs(component), 0.42);
        return MathUtils.Signum(component) * 400.0 * af / (af + 27.13);
    }

    private static Vector3D ChromaticAdaption(Vector3D v)
    {
        return new Vector3D(ChromaticAdaption(v.X), ChromaticAdaption(v.Y), ChromaticAdaption(v.Z));
    }

    private static double HueOf(Vector3D linrgb)
    {
        var scaledDiscount = ScaledDiscountFromLinrgb * linrgb;
        var rgbA = ChromaticAdaption(scaledDiscount);
        // redness-greenness
        var a = (11.0 * rgbA.X + -12.0 * rgbA.Y + rgbA.Z) / 11.0;
        // yellowness-blueness
        var b = (rgbA.X + rgbA.Y - 2.0 * rgbA.Z) / 9.0;
    ...
    private static Vector3D SetCoordinate(
        Vector3D source,
        double coordinate,
        Vector3D target,
        int axis
    ...
    private static Vector3D NthVertex(double y, int n)
    {
        ...
        switch (n)
        {
            ...
            default:
            {
                ...
                return IsBounded(b)
                    ? new Vector3D(coordA, coordB, b)
                    : new Vector3D(-1.0, -1.0, -1.0);
            }
        }
    }

    private static (Vector3D, Vector3D) BisectToSegment(double y, double targetHue)
    {
        var left = new Vector3D(-1.0, -1.0, -1.0);
        var right = left;
        var leftHue = 0.0;
        var rightHue = 0.0;
    ...
    {
        return new Vector3D((a[0] + b[0]) / 2, (a[1] + b[1]) / 2, (a[2] + b[2]) / 2);
    }
    ...
    private static Vector3D BisectToLimit(double y, double targetHue)
    {
    ...
    private static ArgbColor FindResultByJ(double hueRadians, double chroma, double y)
    {
        ...
        var tInnerCoeff =
            1 / Math.Pow(1.64 - Math.Pow(0.29, viewingConditions.BackgroundYToWhitePointY), 0.73);
        ...
        for (var iterationRound = 0; iterationRound < 5; iterationRound++)
        {
            ...
            var linrgb = LinrgbFromScaledDiscount * new Vector3D(rCScaled, gCScaled, bCScaled);
            // ===========================================================
            // Operations inlined from Cam16 to avoid repeated calculation
            // ===========================================================
    ...
    public static ArgbColor SolveToArgb(double hueDegrees, double chroma, double lstar)
    {
